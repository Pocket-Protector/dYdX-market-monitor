<script lang="ts">
  ...
  const fills24h = $derived.by(() => presetToFromTo('24h'));
  const fillsKey = $derived(`/api/fills?slug=${slug}&from=${fills24h.from}&to=${fills24h.to}`);
  ...
  const { data: _fd, isLoading: _fl, error: _fe } = useSWR(() => fillsKey, OPT);
  ...
  $effect(() => {
    const statuses = [
      ...
      { label: 'Fills',     status: tabStatus($_fl, $_fe, $_fd), active: activeTab === 'fills'     },
    ];
